...
static const struct pfa_vendor_ops *vendors[] = {
	&pfa_ghes_ops,
	&pfa_amd_ops,
	&pfa_dsm_ops,
};
			      ...
			      char *buffer, unsigned int size)
{
	...
	int pos = 0, field_num, len;

	if (!rr || !buffer)
		return;

	fields = rr->ops->fields;
	field_num = rr->ops->field_num;
...
{
	if (!rr1 || !rr2 || rr1->ops != rr2->ops)
		return false;

	int field_num = rr1->ops->row_field_num;
...
static void row_record_copy(struct row_record *dst, struct row_record *src)
{
	...
	for (int i = 0; i < PFA_MAX_FIELDS; i++)
...
{
	const struct pfa_vendor_ops *ops = NULL;
	int i;
	...
	for (int idx = 0; idx < ops->field_num; idx++) {
		if (parse_value(detail, ops->fields[idx].anchor_str,
				ops->fields[idx].value_base, &r->location_fields[idx])) {
			log(TERM, LOG_INFO,
...
static void row_record_fn(struct row_record *rr, time_t time)
{
	if (!rr)
		return;
...
void ras_record_row_error(const char *detail, unsigned int count, time_t time, unsigned long long addr)
{
	...
	row_record_fn(pr, time);
}

void row_record_infos_free(void)
{
	struct row_record *rr = NULL, *tmp_rr = NULL;
	struct page_addr *pa = NULL, *tmp_pa = NULL;

	rr = LIST_FIRST(&row_head);
	while (rr) {
		pa = LIST_FIRST(&rr->page_head);
		while (pa) {
			tmp_pa = LIST_NEXT(pa, entry);
			LIST_REMOVE(pa, entry);
			free(pa);
			pa = tmp_pa;
		}
		tmp_rr = LIST_NEXT(rr, entry);
